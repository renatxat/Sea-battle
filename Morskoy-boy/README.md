...
3) Запустите файл main.py и наслаждайтесь игрой.
    ...
    Второй режим пока не реализован. Выбираем первый режим. Далее можно выбрать, кто будет ходить первым, посредством
    нажатия на одну из трёх кнопок. Потом рисуется игровое поле и надпись сверху с просьбой разместить корабль
    того или иного размера. При неправильном выборе клеток корабля выскакивает окно с перезапуском размещения
    только этого корабля. Для полного перезапуска закройте игровое окно и запустие main.py заново. Когда
