...
- Если собираетесь играть по сети, запустите файл server.py при помощи команды:
```python3 server.py```

### Light:
Pyinstaller.
- Если у вас windows, запустите файл sea-battle.exe из папки game/windows
- Eсли у вас ubuntu linux, зайдите в консоли в папку game/linux и пропишите:
```sudo chmod +x sea-battle && ./sea-battle```
- Если у вас macos или другая unix-подобная ОС, то воспользуйтесь способом ниже или самостоятельно
сгенерируйте pyinstaller файл: ```pip install PyInstaller && pyinstaller main.spec```.
Сгенерированный файл будет находиться по адресу dist/sea-battle
- Вы можете перемещать этот исполняющий файл куда угодно, всё будет работать
...
Планы на будущее:
-----
- добавить возможность автоматической расстановки кораблей
...
образом в соотвествии с правила морского боя,
то этот процесс всегда удасться довести до конца.
