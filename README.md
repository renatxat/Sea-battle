...
а также автоматическую расстановку кораблей.
...
объекта AutomaticArrangement из constructor_field.py.
...
Подробнее:
     ...
     2) В Application создаётся объект __foreign_field класса BattlefieldOpponent из battlefield_opponent_view.py.
     Это доска слева, корабли на которой надо сгенерировать при помощи ConstructorFields и передать при создании,
     вы можете жмакать кнопки на ней, когда есть надпись "Ваш ход". Так вы и ходите.
...
для расстановки ваших кораблей или расставляет их автоматически, на этот раз уже с таймером
(все таймеры расписаны в config).
...
и на экране другого игрока. Если оба игрока успеют расставить корабли, то у каждого из них сгенерируется
...
Классы:
------------
0) Класс Window из wrappers.py является обёрткой для основного окна модуля tkinter – Tk.
Он настраивает окна двух типов:
...
1) Класс Button из wrappers.py является обёрткой для кнопок модуля tkinter – Button. Он нужен, чтобы поменять
дефолтные настройки кнопок на те, которые нужны для работы. Дело в том, что на разных устройствах (ОС) они
могут сильно отличаться.

2) Класс Canvas из wrappers.py является обёрткой для Canvas из модуля tkinter. Он нужен для правильной
настройки размеров канваса на разных ОС.

3) Класс Ship создаёт корабли по переданным координатам, в которые можно стрелять посредством метода shot.
...
используется при реализации классов ConstructorFields, AutomaticArrangement.

4) Класс Battlefield выполняют всю логику основного игрового поля. В него можно стрелять при помощи метода
...
5) Класс BattlefieldPlayer является наследником класса Battlefield. Он просто является фасадом для него.
...
6) Класс BattlefieldOpponent создаёт поле в левой части доски (canvas) для игрока с полем противника,
...
8) Класс ConstructorFields из constructor_fields.py. Он либо запрашивает автоматически сгенерированную расстановку у объекта
AutomaticArrangement, либо создаёт поле в окне класс Window, в котором необходимо протыкивать
...
6) Класс AutomaticArrangement из constructor_fields.py генерирует автоматическую расстановку кораблей.
Сделано это при помощи библиотеки random и **некоторой леммы**. Она же используются, по сути, при вводе
расположения кораблей игроком в классе ConstructorFields. С её формулировкой и доказательством
можете ознакомиться в самом конце данного файла. Можно забрать сгенерированную 
...
Примечания:
--------------
    0) Игра просто великолепно работает на windows 10, довольно хорошо на linux ubuntu, более менее на macos и
    вовсе не тестировалась на других операционных системах.
    1) В модуле wrappers.py можно увидеть, что на macos используется библиотке tkmacosx, а не tkinter. Это связано
    с тем, что кнопки на macos в tkinter не меняют цвет фона из-за неудачной реализации.
    2) Некоторые методы, схожие по логике ожидания ответа от сервера, разъединены в классе Client, потому что нельзя
    обновлять окна в модуле tkinter в отдельном потоке, если запущен бесконечный цикл mainloop.
    3) Некоторые потоки продолжают ждать ответ от клиента даже после закрытия окна. Это может происходить, если игрок
    ...
    4) Если вы пытаетесь играть онлайн по одной и той же сети, то вы можете сломать сервер экстренным выходом, если
    ...
    5) В режиме игры с ботом клетки для выстрела можно выбирать во время хода бота, это никак не отображается, но 
    ход будет засчитан после того, как отобразятся все выстрелы за ход бота. Сначала это было багом, теперь это фича!
    6) Можно сделать кастомизированное поле и размеры кораблей в файле config (на pyinstaller это не действует).
